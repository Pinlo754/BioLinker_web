    ...
    switch (el.type) {
      ...
      case "name":
        return (
          <div key={el.id} style={style}>
            <h1
              className={`${
                profileData.globalStyles?.textStyles?.titles ??
                "text-2xl font-bold"
              }`}
      ...
      case "title":
        return (
          <div key={el.id} style={style}>
            <p
              className={`${
                profileData.globalStyles?.textStyles?.headings ??
      ...
      case "divider":
        return (
          <div key={el.id} style={style}>
            <div
              style={{ height: 2, width: "100%", backgroundColor: "#111" }}
            />
          </div>
        );
      ...
      case "bio":
        return (
          <div key={el.id} style={style}>
            <p
              className={`${
                profileData.globalStyles?.textStyles?.paragraphs ??
          ...
          <div key={el.id} style={style}>
            <a
              ...
              onClick={async (e) => {
                ...
                const linkId = el.content?.staticLinkId;
                const url = el.content?.url;
                try {
                  if (linkId) {
                    await fetch(
                      `https://biolinker.onrender.com/api/AnalyticLink/click/${linkId}`,
                      {
                        method: "POST",
                      }
                    );
                  ...
                  console.error("Gửi analytic thất bại:", err);
                } finally {
                  // Dù thành công hay thất bại thì vẫn mở link
              ...
              style={{
                backgroundColor:
                  profileData.globalStyles?.buttonColor ?? "#2d3748",
                color: "#fff",
                textDecoration: "none",
                padding: "10px 18px",
                borderRadius:
    ...
    switch (el.type) {
      ...
      case "avatar":
        return (
          <div
            key={el.id}
            className="w-24 h-24 rounded-full overflow-hidden border-4 border-white shadow-lg mb-2"
          >
            <img
              src={getContentValue(el.content) || "/avatar.png"}
              alt="avatar"
          ...
          <a
            ...
            onClick={async (e) => {
              ...
              try {
                ...
                // Dù request có lỗi hay không vẫn mở link thật
                if (url) window.open(url, "_blank");
              }
            }}
