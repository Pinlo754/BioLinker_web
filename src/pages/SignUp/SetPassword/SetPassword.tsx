...
const SetPassword: React.FC<SetPasswordProps>= ({ visible}) => {
    const { validatePassword } = useSignUp();
    const { password, showPassword, setPassword, setShowPassword, confirmPassword, setConfirmPassword, showConfirmPassword, setShowConfirmPassword } = useSetPassword();
    ...
    return (
        <div className="fixed inset-0 z-[9999] bg-black/25 backdrop-blur-[1px] flex items-center justify-center">
            <div className="bg-white rounded-3xl p-8 shadow-xl flex flex-col items-center gap-4 w-[50%] h-[60%]">
